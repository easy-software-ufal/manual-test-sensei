...
class MisplacedAction(Transformator):
    def __init__(self):
        self.smell = smells_names.MISPLACED_ACTION
        super().__init__()
        ...
        for (step_index, st) in enumerate(test.steps):
            for (reaction_index, reaction) in enumerate(st.reactions):
                ...
                for (match_id, start, end) in reaction_matches:
                    helpers._store_smell(st, smells_names.MISPLACED_ACTION, 'Transformed misplaced action. Location', 'verification', reaction[start:end])
