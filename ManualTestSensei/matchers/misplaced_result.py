        ...
        for step in test.steps:
            ...
            for match_id, token_ids in action_matches:
                for token_id in token_ids:
                    helpers._store_smell(step, self.smell, 'verification performed', 'action', step.action[token_id])

            # Second test: Interrogative sentences as step
            for sentence in step.action.sents:
                if is_interrogative_sentence(sentence):
                    helpers._store_smell(step, self.smell, 'question as step', 'action', sentence)
            ...
            for sentence in step.action.sents:
                if not (is_interrogative_sentence(sentence) or is_imperative_sentence(sentence)):
                    ...
                    for match_id, token_ids in action_matches:
                        ...
                        helpers._store_smell(step, self.smell, 'SUT state declaration', 'action', sentence)
